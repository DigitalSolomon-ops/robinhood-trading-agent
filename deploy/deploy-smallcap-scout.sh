#!/usr/bin/env bash
# Deploy the Small-Cap Scout daily email as a Cloud Run Job + Cloud Scheduler.
...
#   bash deploy/deploy-smallcap-scout.sh
#
...
gcloud builds submit --project="$PROJECT" \
  --config=deploy/cloudbuild.smallcap-scout.yaml \
  --substitutions=_IMAGE="$IMAGE" .

echo "==> Runtime service account + secret access"
if ! gcloud iam service-accounts describe "$SA" --project="$PROJECT" >/dev/null 2>&1; then
  gcloud iam service-accounts create "$SA_NAME" --project="$PROJECT" \
    --display-name="Small-Cap Scout daily job"
...
echo "==> Cloud Scheduler -> run the job daily ($CRON $TZONE)"
